const statusRawat = require('../models/StatusRawat')
const pagination = require('../configs/Pagination')

class StatusRawatController {
    index(req, res) {
        const statusRawatObject = new statusRawat()
        statusRawatObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = StatusRawatController