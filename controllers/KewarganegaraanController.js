const kewarganegaraan = require('../models/Kewarganegaraan')
const pagination = require('../configs/Pagination')

class KewarganegaraanController {
    index(req, res) {
        const kewarganegaraanObject = new kewarganegaraan()
        kewarganegaraanObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = KewarganegaraanController