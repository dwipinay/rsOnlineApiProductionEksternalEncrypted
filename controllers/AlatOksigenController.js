const alatOksigen = require('../models/AlatOksigen')
const pagination = require('../configs/Pagination')

class AlatOksigenController {
    index(req, res) {
        const alatOksigenObject = new alatOksigen()
        alatOksigenObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = AlatOksigenController