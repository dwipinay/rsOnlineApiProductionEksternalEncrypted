const severityLevel = require('../models/SeverityLevel')
const pagination = require('../configs/Pagination')

class SeverityLevelController {
    index(req, res) {
        const severityLevelObject = new severityLevel()
        severityLevelObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = SeverityLevelController