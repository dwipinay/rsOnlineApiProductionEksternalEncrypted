const varianCovid = require('../models/VarianCovid')
const pagination = require('../configs/Pagination')

class VarianCovidController {
    index(req, res) {
        const varianCovidObject = new varianCovid()
        varianCovidObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = VarianCovidController