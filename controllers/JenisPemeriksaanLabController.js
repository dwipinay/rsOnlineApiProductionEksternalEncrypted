const jenisPemeriksaanLab = require('../models/JenisPemeriksaanLab')
const pagination = require('../configs/Pagination')

class JenisPemeriksaanLabController {
    index(req, res) {
        const jenisPemeriksaanLabObject = new jenisPemeriksaanLab()
        jenisPemeriksaanLabObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = JenisPemeriksaanLabController