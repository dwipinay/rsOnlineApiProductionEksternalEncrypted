const jenisVaksin = require('../models/JenisVaksin')
const pagination = require('../configs/Pagination')

class JenisVaksinController {
    index(req, res) {
        const jenisVaksinObject = new jenisVaksin()
        jenisVaksinObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = JenisVaksinController