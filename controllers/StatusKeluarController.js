const statusKeluar = require('../models/StatusKeluar')
const pagination = require('../configs/Pagination')

class StatusKeluarController {
    index(req, res) {
        const statusKeluarObject = new statusKeluar()
        statusKeluarObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = StatusKeluarController