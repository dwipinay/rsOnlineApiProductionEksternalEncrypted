const ketersediaanPelayanan = require('../models/KetersediaanPelayanan')
const pagination = require('../configs/Pagination')
const Joi = require('joi')

class KetersediaanPelayananController {
    index(req, res) {
        ...
        const ketersediaanPelayananObject = new ketersediaanPelayanan()
        ketersediaanPelayananObject.getData(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = KetersediaanPelayananController