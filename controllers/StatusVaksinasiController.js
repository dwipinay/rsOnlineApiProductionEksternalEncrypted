const statusVaksinasi = require('../models/StatusVaksinasi')
const pagination = require('../configs/Pagination')

class StatusVaksinasiController {
    index(req, res) {
        const statusVaksinasiObject = new statusVaksinasi()
        statusVaksinasiObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = StatusVaksinasiController