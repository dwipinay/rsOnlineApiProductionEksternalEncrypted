const kelompokGejala = require('../models/KelompokGejala')
const pagination = require('../configs/Pagination')

class KelompokGejalaController {
    index(req, res) {
        const kelompokGejalaObject = new kelompokGejala()
        kelompokGejalaObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = KelompokGejalaController