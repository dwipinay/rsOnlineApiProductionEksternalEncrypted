const komorbid = require('../models/Komorbid')
const pagination = require('../configs/Pagination')

class KomorbidController {
    index(req, res) {
        const komorbidObject = new komorbid()
        komorbidObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = KomorbidController