...
const klaimKemKes = require('../models/KlaimKemKes')
const pagination = require('../configs/Pagination')

class KlaimKemKesController {
    index(req, res) {
        const schema = Joi.object({
            no_tpkd: Joi.string().required()
        })
        ...
        const klaimKemKesObject = new klaimKemKes()
        klaimKemKesObject.getData(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = KlaimKemKesController