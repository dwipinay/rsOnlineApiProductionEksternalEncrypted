const jenisPasien = require('../models/JenisPasien')
const pagination = require('../configs/Pagination')

class JenisPasienController {
    index(req, res) {
        const jenisPasienObject = new jenisPasien()
        jenisPasienObject.getAll(req, (err, results) => {
            if (err) {
                res.status(422).send({
                    status: false,
                    message: err
...
module.exports = JenisPasienController